...
int printMenu(float kilometers, float aerolineasFee, float latamFee) {
	...
	if(aerolineasFee > 0) {
		...
		printf("- Precio vuelo Aerolíneas: Aún no se ha ingresado ningún valor.\n");
	}
	if(latamFee > 0) {
		printf("- Precio vuelo Latam: $%.2f.\n", latamFee);
	} else {
		printf("- Precio vuelo Latam: Aún no se ha ingresado ningún valor.\n");
	}
...
int getOption(int *pNumber, int minimumValue, int maximumValue, int tries) {
	...
	while ((aux < minimumValue || aux > maximumValue) && tries > 0) {
		printf("La opción ingresada no es correcta.\nPor favor, ingrese una opción entre %d y %d. Usted tiene %d intentos.", minimumValue, maximumValue, tries);
		scanf("%d", &aux);
		tries--;
	}
...
int getKilometers(float *pKilometers, int minimumValue, int maximumValue, int tries) {
	...
	tries--;
	while ((aux < minimumValue || aux > maximumValue) && tries > 0) {
		printf("El valor de kilómetros ingresado no es correcto.\nPor favor, ingrese un nuevo valor entre %d y %d kilómetros. Usted tiene %d intentos.", minimumValue, maximumValue, tries);
		scanf("%f", &aux);
		tries--;
	}
	if (aux >= minimumValue && aux <= maximumValue) {
